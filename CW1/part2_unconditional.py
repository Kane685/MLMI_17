...
def visualize_forward_diffusion(dataloader, alphas_cumprod, n_steps=10):
    ...
    plt.axis("off")
    plt.savefig("images_unconditional/forward_diffusion_process.png")
    plt.show()
...
if __name__ == "__main__":
    Path("images_unconditional").mkdir(exist_ok=True)
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 5) % 5 == 0:
            ...
            # Save the result
            vutils.save_image(grid, f"images_unconditional/evolution_epoch_{epoch+1}.png")
            print(
                f"Epoch {epoch+1}: Saved generation grid to images_unconditional/evolution_epoch_{epoch+1}.png"
            )
    ...
    vutils.save_image(final_grid, "images_unconditional/dit_mnist_final.png")

    # Save Loss Plot
    plt.figure()
    plt.plot(loss_history)
    plt.title("Training Loss")
    plt.savefig("images_unconditional/loss_curve.png")

    print("Done! Check the 'images_unconditional' folder.")
