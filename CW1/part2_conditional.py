...
@torch.no_grad()
def sample_ddpm(net, T, bsz, betas, alphas, alphas_cumprod, num_snapshots=10, labels = None):
    net.eval()
...
def visualize_forward_diffusion(dataloader, alphas_cumprod, n_steps=10):
    ...
    plt.axis("off")
    plt.savefig("images_conditional/forward_diffusion_process.png")
    plt.show()
...
if __name__ == "__main__":
    Path("images_conditional").mkdir(exist_ok=True)
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 5) % 5 == 0:
            ...
            # Save the result
            vutils.save_image(grid, f"images_conditional/evolution_epoch_{epoch+1}.png")
            print(
                f"Epoch {epoch+1}: Saved generation grid to images_conditional/evolution_epoch_{epoch+1}.png"
            )
    ...
    trajectory, labels_cpu = sample_ddpm(
        net, T, bsz=10, betas=betas, alphas=alphas, alphas_cumprod=alphas_cumprod, labels=labels
    )
    final_imgs = trajectory[-1]  # [10, 1, H, W]
    fig, axes = plt.subplots(1, 10, figsize=(20, 3))
    for i, ax in enumerate(axes):
        img = final_imgs[i].squeeze(0).clamp(-1, 1)
        img = (img + 1) / 2  # [-1,1] -> [0,1]
        ax.imshow(img.numpy(), cmap="gray")
        ax.set_title(f"label={int(labels_cpu[i])}")
        ax.axis("off")
    plt.tight_layout()
    plt.savefig("images_conditional/dit_mnist_final_labeled.png")

    # Save Loss Plot
    plt.figure()
    plt.plot(loss_history)
    plt.title("Training Loss")
    plt.savefig("images_conditional/loss_curve.png")

    print("Done! Check the 'images_conditional' folder.")
