...
def visualize_samples(
    ...
    plt.tight_layout()
    plt.savefig(Path("figures") / f"part1_{split_name}_samples.png")
    plt.close()
...
def get_features(
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        shuffle = False,
        num_workers = num_workers # read data in advance
    )
    ...
    with torch.no_grad(): # frozen backbone
        for images, _ in dataloader:
            ...
            list.append(features_batch.cpu().numpy())
    features = np.concatenate(list, axis = 0)
...
def visualize_features_tsne(
    ...
    plt.tight_layout()
    plt.savefig(Path("figures") / f"part1_{title.replace(' ', '_').lower()}.png")
    plt.close()
...
def train_linear_probe(
    ...
    # define probe
    dim_in = features_dataset.features.shape[1] # get dimension of features
    dim_out = features_dataset.num_classes
...
def train_finetune_probe(
    ...
    optimizer_head = torch.optim.Adam(
        pretrained_linear_probe.parameters(), 
        lr= head_lr, 
        weight_decay= weight_decay
    )
    ...
    pretrained_linear_probe.to(device)
    feature_extractor.train()
    pretrained_linear_probe.train()    
    for epoch in range(num_epochs):
        total_loss = 0
        count = 0
        for x, y in loader:
            ...
            output = pretrained_linear_probe(feature)
            loss = cost(output, y)
            loss.backward()
            ...
            total_loss += loss.item() * x.size(0)
        epoch_losses.append(total_loss/count)
    model = nn.Sequential(feature_extractor, pretrained_linear_probe)
...
def plot_losses(losses, name):
    ...
    plt.tight_layout()
    plt.savefig(Path("figures") / f"part1_{name}_training_loss.png")
    plt.show()
    plt.close()
...
def evaluate_linear(
    ...
    probe.eval()
    with torch.no_grad():
        num_total = features.features.shape[0]
        num_correct = 0
        for x, y in loader:
            x = x.to(device)
            y = y.to(device)
            ...
            num_correct += (preds == y).sum().cpu().item()
...
def evaluate_finetune(
    ...
    dataset = FruitDataset(split_name, transform=extractor.transform)
    loader = torch.utils.data.DataLoader(
        dataset,
        batch_size = batch_size,
        shuffle = False,
    ...
    model.eval()
    with torch.no_grad():
        num_total = dataset.images.shape[0]
        num_correct = 0
        for x, y in loader:
            x = x.to(device)
            y = y.to(device)
...
def train_finetune_probe_acc(
    split_name: str,
    feature_extractor: torch.nn.Module,
    pretrained_linear_probe: torch.nn.Module = None,
    num_epochs: int = 10,
    batch_size: int = 64,
    feature_lr: float = 2e-5,
    ...
    for epoch in range(num_epochs):
        ...
        for x, y in loader:
            ...
            total_loss += loss.item() * x.size(0)
        epoch_losses.append(total_loss/count)
        print(f"[finetune] epoch {epoch+1}/{num_epochs}, loss={total_loss/count:.4f}")
    model = nn.Sequential(feature_extractor, pretrained_linear_probe)
